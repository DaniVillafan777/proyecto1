...
class Complementos extends FPDF
{
	...
	function Row($data, $fill = 'D')
	{
		...
		if (isset($this->type_cell)) {
			if (in_array('m', $this->type_cell) || in_array('wt', $this->type_cell)) {
				for ($i = 0; $i < count($data); $i++) {
					$d = ($this->type_cell[$i] == 'wt') ? str_replace($this->tag()[0], $this->tag()[1], $data[$i]) : $data[$i];
		...
		for ($i = 0; $i < count($data); $i++) {
			...
			$y = $this->GetY();
			//Draw the border
			if (isset($this->borderRow[$i])) {
				$fill = ($fill == 'DF' || $fill == 'DF') ? true : false;
				$this->DibujarBorde($w, $h, $this->borderRow[$i], $fill);
			} else {
	...
	function Parser($text)
	{
		...
		// $begin = strlen($regs[1]) ;
		$begin = strlen(isset($regs[1]) ? $regs[1] : 3);
		$end = strlen($text);
